...
class Agent(object):
    ...
    DECELERATION_SPEEDS = {
        ...
        'normal': 0.6,  # Faster deceleration than 'slow'
        'fast': 0.3     # Fastest deceleration
    }

    def __init__(self, world=None, scale=30.0, mass=1.0, max_speed=2500.0, mode='seek'):
...
##            force = self.pursuit(self.world.hunter)
    ...
    def flee(self, targetPos):
        from_target = self.pos - targetPos
        distance = from_target.length()
        panicDistance = 100  # Define how close the agent must be to start fleeing
        if distance < panicDistance:
            desiredVel = from_target.normalise() * self.max_speed
            return (desiredVel - self.vel)
        
        return Vector2D()  # No fleeing if outside panic distance
