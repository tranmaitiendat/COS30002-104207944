...
class GameWindow(pyglet.window.Window):
	...
	def __init__(self, **kwargs):
		...
		self.labels = {
			'mode':	pyglet.text.Label('', x=40, y=self.height-20, color=COLOUR_NAMES['WHITE']),
			'status':	pyglet.text.Label('', x=50, y=20, color=COLOUR_NAMES['WHITE']),
			'max speed':	pyglet.text.Label('', x=240, y=self.height-20, color=COLOUR_NAMES['WHITE']),
			'mass':	pyglet.text.Label('', x=450, y=self.height-20, color=COLOUR_NAMES['WHITE']),
			'agents number':	pyglet.text.Label('', x=650, y=self.height-20, color=COLOUR_NAMES['WHITE']),
		}
