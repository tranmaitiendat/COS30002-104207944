...
class GameWindow(pyglet.window.Window):
	...
	def __init__(self, **kwargs):
		...
		self.labels = {
			'prey collision range':		pyglet.text.Label('', x=50, y=self.height-20, color=COLOUR_NAMES['WHITE']),
			'prey health':		pyglet.text.Label('', x=350, y=self.height-20, color=COLOUR_NAMES['WHITE']),
			'prey first target':		pyglet.text.Label('', x=650, y=self.height-20, color=COLOUR_NAMES['WHITE']),
			'prey second target':		pyglet.text.Label('', x=1000, y=self.height-20, color=COLOUR_NAMES['WHITE']),
			'hunter mode':		pyglet.text.Label('', x=200, y=20, color=COLOUR_NAMES['WHITE']),
			'inaccurate rate':		pyglet.text.Label('', x=550, y=20, color=COLOUR_NAMES['WHITE']),
			'eta time to collide':		pyglet.text.Label('', x=900, y=20, color=COLOUR_NAMES['WHITE']),
		}
